...
const LogoutDialog = () => {
  ...
  const { dark } = useTheme();
  const { action, logoutVisible, title } = useSelector(dialogSelector) as DialogState;
  ...
  return (
    <Portal>
      ...
      >
        ...
        <Dialog.Content style={styles.content}>{dark ? <LogoutIconAlt /> : <LogoutIcon />}</Dialog.Content>
        <Dialog.Actions>
          <Button onPress={onDismiss} uppercase={false} theme={dark ? combinedDarkTheme : combinedDefaultTheme}>
            Cancel
          </Button>
          <Button onPress={onAction} uppercase={false} theme={dark ? combinedDarkTheme : combinedDefaultTheme}>
            Yes
          </Button>
        </Dialog.Actions>
      </Dialog>
